    ...
    : UserControl
{
    ...
    private void CanvasControl_Paint(object sender, PaintEventArgs e)
    {
        ...
        if (group is not null)
        {
            ...
            {
                shape.DrawShape<double>(g, panPoint, scale);
            }
        }
    ...
    {
        var translateScalePoint = ScreenToObjectTransposedMatrix(panPoint.ToTuple<double>(), e.Location.ToTuple<double>(), scale).ToPointF();
        var scalePoint = ScreenToObject(e.Location.ToTuple<double>(), scale).ToPointF();

        if (e.Button == MouseButtons.Middle && !panning)
        {
            panning = true;
            startingPoint = translateScalePoint;
            panPoint = ScreenToObjectTransposedMatrix(startingPoint.ToTuple<double>(), e.Location.ToTuple<double>(), scale).ToPointF();
    ...
    {
        //var mousePoint = ScreenToObjectTransposedMatrix(panPoint.ToTuple<double>(), e.Location.ToTuple<double>(), scale).ToPointF();
        //Cursor =
    ...
    private void CanvasControl_MouseWheel(object sender, MouseEventArgs e)
    {
        ...
        panPoint = ZoomAtTransposedMatrix(panPoint.ToTuple<double>(), e.Location.ToTuple<double>(), previousScale, scale).ToPointF();
